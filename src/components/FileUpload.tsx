...
export default function FileUpload({ onSuccess }: FileUploadProps) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!data.daily || !data.totals) {
          throw new Error("잘못된 cc.json 형식입니다");
        }

        setParsedData(data);
        setUploadState("idle");
      } catch (error) {
        setErrorMessage("잘못된 파일 형식입니다. 유효한 cc.json 파일을 업로드해주세요.");
  ...
  const handleSubmit = async () => {
    ...
    if (!session?.user?.username) {
      setErrorMessage("GitHub 로그인이 필요합니다.");
      setUploadState("error");
      return;
    }
    ...
    <div>
      ...
      >
        ...
        <p className="text-sm text-foreground">
          {isDragActive ? "여기에 놓으세요" : "cc.json 파일을 드래그하거나 클릭하세요"}
        </p>
      </div>
